...
%%%-------------------------------------------------------------------
-module(plApi).
-export([connect_websocket/2, create_session/1]).
	...
	{ok, WebSocket} ->
		...
		{ok, Messages} ->
			...
			[<<"client-connected">>] ->
			    ...
			    SyncServer = gen_server:call(plRegistry, {new_client, Sessionid, self()}),
...
listen_loop(WebSocket, SyncServer) ->
	...
	{tcp, WebSocket, DataFrame} ->
        ...
        gen_fsm:send_event(SyncServer, {received, Lin}),
        listen_loop(WebSocket, SyncServer);
...
%%--------------------------------------------------------------------
%% Function: create_session(CallbackServer) -> {ok, SessionId} |
%%                                     {error, Reason}
%%--------------------------------------------------------------------
create_session(CallbackServer) ->
    gen_server:call(plRegistry, {new_session, CallbackServer}).
